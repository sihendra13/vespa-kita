  ...
  const eventsHtml = (eventRows || [])
    .map((e) => {
      ...
      const sponsorBlock = sponsorLogos.length
        ? `<div style="margin-top:16px; padding-top:16px; border-top:1px solid rgba(241,232,214,0.08);">` +
          `<div style="font-family:var(--mono); font-size:11px; letter-spacing:0.05em; color:var(--chrome); text-transform:uppercase; margin-bottom:12px;">Disponsori oleh</div>` +
          `<div style="display:flex; align-items:center; gap:12px; flex-wrap:wrap;">` +
            ${sponsorLogos.map((url) => {
              const isNorthy = url.toLowerCase().includes("northy");
              const isUnlock = url.toLowerCase().includes("unlock");
              let extraStyle = "";
              if(isNorthy) extraStyle = "filter: brightness(0); height: 18px;";
              if(isUnlock) extraStyle = "height: 44px; transform: scale(1.2);";
