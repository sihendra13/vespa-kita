(function() {
  ...
  banner.innerHTML = `
    <div style="display:flex; align-items:center; gap:12px; background:var(--aspal-2, #1E2124); padding:12px 16px; border-radius:12px; border:1px solid rgba(255,255,255,0.6); box-shadow:0 10px 30px rgba(0,0,0,0.8);">
      ...
      <div style="flex:1;">
        ...
        <div style="font-size:12px; color:#FFFFFF; line-height:1.3; margin-top:4px;">${t('Instal aplikasi VespaKita untuk pengalaman yang lebih menyenangkan.', 'Install the VespaKita app for a smoother experience.')}</div>
      </div>
  ...
  iosModal.innerHTML = `
    <div id="pwa-ios-modal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.85); z-index:10000; align-items:center; justify-content:center; padding:20px; backdrop-filter:blur(5px);">
      <div style="background:var(--aspal, #15171A); padding:32px 24px; border-radius:16px; width:100%; max-width:320px; text-align:center; position:relative; border:1px solid rgba(255,255,255,0.1); box-shadow:0 10px 40px rgba(0,0,0,0.5);">
        ...
        <div style="font-family:var(--display, sans-serif); font-size:20px; margin-bottom:8px; color:#fff; text-transform:uppercase;">${t('Install Aplikasi', 'Install App')}</div>
        <p style="color:var(--chrome, #A9A49B); font-size:14px; margin-bottom:24px; line-height:1.6;">${t('Agar lebih praktis, ikuti 2 langkah mudah ini:', 'For a smoother experience, follow these 2 easy steps:')}</p>
        ...
        <div style="background:rgba(255,255,255,0.05); padding:16px; border-radius:12px; text-align:left;">
          <div style="color:#fff; font-size:13px; margin-bottom:12px; display:flex; align-items:flex-start; gap:12px;">
            ...
            <div>${t('Tekan tombol', 'Tap the')} <b>Share</b> <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:text-bottom; margin:0 2px;"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"></path><polyline points="16 6 12 2 8 6"></polyline><line x1="12" y1="2" x2="12" y2="15"></line></svg> ${t('di bar bagian bawah Safari.', 'button in the Safari toolbar.')}</div>
          </div>
          <div style="color:#fff; font-size:13px; display:flex; align-items:flex-start; gap:12px;">
            ...
            <div>${t('Geser menu ke atas, lalu pilih', 'Scroll the menu up, then choose')} <b>${t('Tambah ke Layar Utama', 'Add to Home Screen')}</b> <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="vertical-align:text-bottom; margin:0 2px;"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><line x1="12" y1="8" x2="12" y2="16"></line><line x1="8" y1="12" x2="16" y2="12"></line></svg></div>
          </div>
        </div>
      </div>
    </div>
  ...
  document.getElementById('pwa-install-btn').addEventListener('click', async () => {
    if (isIOS) {
      ...
      alert(t("Gunakan menu browser (titik tiga) lalu pilih 'Install App' atau 'Add to Home Screen'.", 'Use the browser menu (three dots) then choose "Install App" or "Add to Home Screen".'));
    }
  });
})();
